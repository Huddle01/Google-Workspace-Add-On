type TokenFormat = "json" | "xml" | "form-data";
...
interface ServiceOptions {
  ...
  tokenHeaders?: { [key: string]: string } | null;
  tokenMethod?: "get" | "post" | "put" | "delete";
  expirationMinutes?: number;
}
const EXPIRATION_BUFFER_SECONDS_: number = 60;
const LOCK_EXPIRATION_MILLISECONDS_: number = 30 * 1000;

class Service_ {
  private serviceName_: string;
  private params_: ServiceParams;
  private tokenFormat_: TokenFormat;
  private tokenHeaders_: { [key: string]: string } | null;
  private tokenMethod_: "get" | "post" | "put" | "delete";
  private expirationMinutes_: number;
  ...
  constructor(serviceName: string, options?: ServiceOptions) {
    ...
    this.params_ = {};
    this.tokenFormat_ = (options && options.tokenFormat) || "json";
    this.tokenHeaders_ = (options && options.tokenHeaders) || null;
    this.tokenMethod_ = (options && options.tokenMethod) || "post";
    this.expirationMinutes_ = (options && options.expirationMinutes) || 60;
  }
   ...
   * @return {!Service_} This service, for chaining.
   */
  setTokenMethod = function (tokenMethod) {
   ...
   * @return {!Service_} This service, for chaining
   */
  setCodeVerififer = function (codeVerifier) {
   ...
   * @return {!Service_} This service, for chaining
   */
  setCodeChallengeMethod = function (method) {
   ...
   * @return {!Service_} This service, for chaining.
   */
  setAdditionalClaims = function (additionalClaims) {
   ...
   */
  getAuthorizationUrl = function (optAdditionalParameters) {
    validate_({
...
var respContentText = response.getContentText();
   ...
   * @private
   */
  ensureExpiresAtSet_ = function (token) {
   ...
   */
  getToken = function (optSkipMemoryCheck) {
  ...
  isExpired_ = function (token) {
    ...
    if (token.id_token) {
      ...
      if (payload.exp && payload.exp - now < EXPIRATION_BUFFER_SECONDS_) {
        expired = true;
      }
    }
  ...
  canRefresh_ = function (token) {
    ...
    if (token.refreshTokenExpiresAt === undefined) {
      ...
      var now = getTimeInSeconds_(new Date());
      return token.refreshTokenExpiresAt - now > EXPIRATION_BUFFER_SECONDS_;
    }
  };
